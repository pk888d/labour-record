...
// A fresh (CI-seeded) DB has no cycles, so "cycles list: Delete button is
// present" below has nothing to find. Bootstrap one for DNV Orthocare
// (idempotent; reused if already present) and clean it up only if we created it.
const DNV_ESTABLISHMENT_ID = 'est_hospital_dnv'
const CYCLE_YEAR = 2081
const CYCLE_MONTH = 6

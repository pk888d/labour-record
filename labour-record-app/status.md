...
### Task Update — 2026-07-05 14:10 IST
- Task: Add live search to the Help page; add real screenshots, examples, and use-cases to every section
- Status: completed
- Scope: Converted `/help` from a static server component into a client component (`'use client'`) with a search box: content is now a data-driven array of 17 sections (title/path/keywords/body), and typing in the search box live-filters to matching sections (title/path/keyword match) which auto-expand, while non-matching sections hide — clearing the box restores normal manual-toggle `<details>` behaviour. Captured 21 real screenshots of the running app via a standalone Playwright script (seeded the local dev DB with `prisma db push` + `prisma db seed`, created a sample cycle via the API) covering Dashboard, Establishments (list + edit), Employees (list + edit + import), Holidays, Cycles (new + detail), all 4 Form Data Entry tabs (Attendance/Wage Data/Overtime/Leave), Salary Slips (grid + individual original/duplicate), a Print Form (Form XI), Calendar, Settings, Audit Log, and Exports — saved to `public/help/*.png` and embedded inline via a `Shot` component (click-to-open-full-size). Added an `Example` and `UseCase` callout component and wrote a concrete worked example plus a realistic use-case for every major section (e.g. the ESI-threshold example, the mid-month-hire Sync Employees use case, the bulk-export-edit-reimport raise workflow).
- Files changed: src/app/help/page.tsx (rewritten), public/help/*.png (21 new screenshot files), .gitignore (excluded local screenshot-capture script directory)
- Metrics impact: none (same route, now client-rendered with static image assets)
- Validation: `npx tsc --noEmit` — 0 new errors (same 1 pre-existing unrelated e2e error); `npx vitest run` — 92/92 passed; `npm run build` — success (`/help` still prerenders); manually verified via Playwright against the running dev server that typing "ESI" filters to 3 matching sections with screenshots visible, and that a no-match query shows the empty state
- Next step: None — feature complete

### Task Update — 2026-07-08 06:45 IST
- Task: TEC-40/41/42/43/44 — gross wages wrong, "31 days" default, missing attendance automation, NaN console error, misleading compliance warning
- Status: completed (TEC-40/41/42/43 fixed; TEC-44 investigated and closed as working-as-intended, no code change)
- Scope:
  - **TEC-40 (gross wages wrong):** `computeCycleWages` (cycle-wage.ts) was paying Basic/DA straight off `defaultTotalSalary`, unprorated by attendance, while the interactive Wage Data tab correctly prorated by `wageDays/daysInMonth` — the two disagreed depending on which path last wrote `WageRecord`. Fixed by prorating Basic/DA/HRA in `computeCycleWages` using the same `wageDays/daysInMonth` formula, and recomputing PF (on prorated basic+da) and ESI (on prorated gross) instead of the unprorated `computeSalaryBreakdown` figures.
  - **TEC-41 ("31 days" default):** Root cause was `computeCycleWages` falling back to `daysWorked = daysInMonth` whenever no attendance was supplied — which was every new cycle, since `AttendanceRecord` rows didn't exist until a user opened the Attendance tab. Resolved as a side effect of TEC-42 (attendance is now auto-generated at cycle creation, so real attendance is always available).
  - **TEC-42 (attendance automation):** Cycle creation (`POST /api/cycles`) now auto-generates an `AttendanceRecord` per employee using the existing `applyRotatingAttendanceDefaults` "6 working days + 1 holiday" round-robin algorithm, staggered per employee and rotated month-to-month by folding the cycle's month number into each employee's rotation offset (no dependency on a stored "previous cycle" link — deterministic from `(employeeIndex + month)`). Also calls `ensureHolidays(year)` (idempotent) so the govt holiday calendar is seeded before defaults are generated. The wage-seeding step now passes this generated attendance into `computeCycleWages`, so `WageRecord` is correct from the moment a cycle is created.
  - **TEC-43 (NaN console error):** `cycle-form.tsx` (Year, Wage Period Days) and `establishment-form.tsx` (Fixed Allowance, HRA, LWF Rate) bound numeric state directly to `parseInt`/`parseFloat` with no guard — clearing the input produced `NaN` fed back into a controlled `value`. Fixed by allowing these fields to hold an empty string while editing (`cycle-form.tsx`) or `undefined` rendered as `''` (`establishment-form.tsx`), avoiding both the NaN warning and a premature snap-to-0 while typing.
  - **TEC-44 (compliance warning):** Investigated `preflight.ts`'s `FORMS_PENDING` check — found it is deliberate, already-tested behavior (explicit e2e coverage in `26-preflight.spec.ts`, ticket TEC-31, asserts a fresh cycle should show exactly this warning). Not a bug; closed with explanation, no code change.
  - Extracted plan reviewed by Codex (`codex:rescue`) before implementation, which caught two issues fixed in the final approach: (1) proration must use `wageDays` (worked+leave+holiday) not just `daysWorked`, and PF/ESI must be recomputed on the prorated wage, not left at the unprorated figure; (2) the `|| 0` NaN-guard would cause inputs to snap to 0 mid-edit — used empty-string/undefined state instead.
- Files changed: src/domain/calculations/cycle-wage.ts, src/domain/calculations/cycle-wage.test.ts, src/app/api/cycles/route.ts, src/components/cycle-form.tsx, src/components/establishment-form.tsx, e2e/26-preflight.spec.ts
- Metrics impact: none (no schema/API shape change)
- Validation: `npx vitest run` — 202/202 passed; `npx tsc --noEmit` — 0 errors; `npx playwright test` (full suite) — 245/245 passed. RED→GREEN iteration: initial run had 1 unit test failure (cycle-wage.test.ts, stale expectation predating the proration fix — updated to reflect correct prorated math) and 1 e2e failure (26-preflight.spec.ts expected NO_ATTENDANCE warning on a fresh cycle — now correctly absent since attendance auto-generates at creation); both fixed and suite re-run to green.
- Next step: None — all 5 tickets (TEC-40..44) resolved; Linear updated with evidence and closed.
